...
public class CityEnv : EnvBase
{
    ...
    protected override void GenerateDigits()
    {
        ...
        for (var w = 0; w < words.Count; w++)
        {
            ...
            //add all walkable edges
            //edge type follows the second node

            subPath = SmoothenAngles(subPath);
            subPath = SmoothenAngles(subPath);
            // var (start, end, arcs) = getDynamicPath(subPath.Select(p => p.TakeXZ()).ToList());
            ...
            var subPathE = subPath.AsEnumerable().GetEnumerator();
            ...
            for (var lw = 0; lw < fillableWords; lw++)
            {
                ...
                // var digitStartPoint = subPathE.Current;
                var digitStartPoint = GetPointOnPath(subPath, passedDistance);
                // var digitStartPoint = GetPointOnPath(start, end, arcs, passedDistance);
